...
if __name__ == "__main__":
    if not USERNAME or not PASSWORD:
        print("你没有添加任何账户")
        bot.send_message(chat_id=CHAT_ID, text="你没有添加任何账户")
        exit(1)
    user_list = USERNAME.strip().split()
    passwd_list = PASSWORD.strip().split()
    if len(user_list) != len(passwd_list):
        print("The number of usernames and passwords do not match!")
        bot.send_message(chat_id=CHAT_ID, text="The number of usernames and passwords do not match!")
        exit(1)
        ...
        for k, v in SERVERS.items():
            if v:
                if not renew(sessid, s, passwd_list[i], k):
                    ...
                    bot.send_message(chat_id=CHAT_ID, text="ServerID: %s Renew Error!" % k)

                else:
                    print("ServerID: %s has been successfully renewed!" % k)
                    bot.send_message(chat_id=CHAT_ID, text="ServerID: %s has been successfully renewed!" % k)
                    
            else:
                print("ServerID: %s does not need to be renewed" % k)
                bot.send_message(chat_id=CHAT_ID, text="ServerID: %s does not need to be renewed" % k)
